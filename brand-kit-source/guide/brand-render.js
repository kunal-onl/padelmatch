// ═══════════════════════════════════════════════════════════════════
//  PADELMATCH · BRAND BOOK RENDERER
//  Builds all dynamic sections + wires download buttons.
// ═══════════════════════════════════════════════════════════════════
...
(function(PM) {
  ...
  document.getElementById('cover-lockup').innerHTML = PM.buildLockupHTML({
    ...
    specialId: 'clean',
    markSize: 140,
    // wordmarkSize auto-computed to match mark height
    onInk: true,
    showTagline: true,
  ...
  // ──────────────────────────────────────────────────────────────
  //  01 · LOGO MARK — 3 background variants
  // ──────────────────────────────────────────────────────────────
  const markCfgs = [
    { title: 'DEFAULT · CREAM', sub: 'recommended ground',  slug: 'padelmatch-mark-cream', bg: 'cream',     opts: { cellFills: { TL: C.lime, BR: C.blue }, bgColor: C.cream } },
    { title: 'DEFAULT · INK',   sub: 'on dark surfaces',     slug: 'padelmatch-mark-ink',   bg: 'ink',       opts: { cellFills: { TL: C.lime, BR: C.blue }, onInk: true, bgColor: C.ink } },
  ...
  // ──────────────────────────────────────────────────────────────
  //  03 · WORDMARK — PADEL COLOR VARIANT (new section)
  // ──────────────────────────────────────────────────────────────
  ...
  colorVariants.forEach(cv => {
    const tile = document.createElement('div');
    tile.className = 'wm-tile';
    const wm = PM.buildWordmarkHTML({
      caseId: 'pm-split', weightId: 'w900', specialId: 'clean',
      size: 56,
      padelColor: cv.padelColor,
    });
    tile.innerHTML = `
      <div class="wm-stage">${wm}</div>
      <div class="wm-meta">
  ...
  PM.WORDMARK_WEIGHTS.forEach((ww, i) => {
    const tile = document.createElement('div');
    tile.className = 'wm-tile';
  ...
  PM.WORDMARK_SPECIAL.forEach((ws, i) => {
    const tile = document.createElement('div');
    tile.className = 'wm-tile';
    // glyphs variant needs smaller font so the mini-mark replacements (wider than letters) don't overflow
    const sz = ws.id === 'glyphs' ? 32 : WORDMARK_SIZE;
  ...
  const lockupCfgs = [
    {
      title: 'HORIZONTAL · DEFAULT', sub: 'app bar · header · footer',
      slug: 'padelmatch-lockup-horizontal',
      bg: 'cream',
      // wordmarkSize omitted → auto-computed to match mark height
      lockupOpts: { orientation: 'horizontal', markSize: 96 },
    },
    {
      title: 'HORIZONTAL · INK', sub: 'dark mode · video stinger',
      slug: 'padelmatch-lockup-horizontal-ink',
      bg: 'ink',
      lockupOpts: { orientation: 'horizontal', markSize: 96, onInk: true },
    },
    {
      title: 'STACKED · CENTERED', sub: 'square spaces · app store',
      slug: 'padelmatch-lockup-stacked',
      bg: 'cream',
      lockupOpts: { orientation: 'stacked', markSize: 120, wordmarkSize: 44 },
    },
    {
      title: 'STACKED · WITH TAGLINE', sub: 'marketing · hero panels',
      slug: 'padelmatch-lockup-stacked-tagline',
      bg: 'cream',
      lockupOpts: { orientation: 'stacked', markSize: 120, wordmarkSize: 40, gap: 10, showTagline: true, tagline: 'Find the perfect match.' },
    },
    {
      title: 'MARK ONLY', sub: 'compact · favicon · nav',
      slug: 'padelmatch-mark-only',
      bg: 'cream',
      lockupOpts: { orientation: 'mark', markSize: 140 },
    },
    {
      title: 'WORDMARK ONLY', sub: 'text-heavy contexts',
      slug: 'padelmatch-wordmark-only',
      bg: 'cream',
      lockupOpts: { orientation: 'wordmark', wordmarkSize: 56 },
    },
    {
      title: 'HORIZONTAL · WITH TAGLINE', sub: 'business cards · sign-offs',
      slug: 'padelmatch-lockup-horizontal-tagline',
      bg: 'cream',
      // wordmarkSize omitted → auto-fits wordmark + tagline to mark height
      lockupOpts: { orientation: 'horizontal', markSize: 120, showTagline: true, tagline: 'Find the perfect match.' },
    },
    {
      title: 'STACKED · ON INK', sub: 'splash · dark hero',
      slug: 'padelmatch-lockup-stacked-ink',
      bg: 'ink',
      lockupOpts: { orientation: 'stacked', markSize: 120, wordmarkSize: 40, gap: 10, onInk: true, showTagline: true, tagline: 'Find the perfect match.' },
    },
  ];
  ...
  const iconCfgs = [
    {
      title: 'FAVICON · 48 PX', sub: 'browser tab · light',
      slug: 'padelmatch-favicon-48',
      stage: 'light',
      build: () => PM.buildFaviconSVG({ size: 48, bgColor: C.cream }),
      sizes: [16, 32, 48],
    },
    {
      title: 'FAVICON · DARK · 48 PX', sub: 'browser tab · dark',
      slug: 'padelmatch-favicon-48-dark',
      stage: 'dark',
      build: () => PM.buildFaviconSVG({ size: 48, bgColor: C.ink }).replace(/fill="#111118" fill-rule="evenodd"/, 'fill="#FFFFFF" fill-rule="evenodd"'),
      sizes: [16, 32, 48],
    },
    {
      title: 'APP ICON · CREAM', sub: 'ios · android · 1024',
      slug: 'padelmatch-appicon-cream',
      stage: 'light',
      build: () => PM.buildAppIconSVG({ size: 1024, bgColor: C.cream }),
      sizes: [1024, 512, 180],
    },
    {
      title: 'APP ICON · LIME', sub: 'high-energy variant',
      slug: 'padelmatch-appicon-lime',
      stage: 'light',
      build: () => PM.buildAppIconSVG({ size: 1024, bgColor: C.lime, cellFills: { TL: C.ink, BR: C.blue } }),
      sizes: [1024, 512, 180],
    },
    {
      title: 'APP ICON · INK', sub: 'dark-mode springboard',
      slug: 'padelmatch-appicon-ink',
      stage: 'dark',
      build: () => PM.buildAppIconSVG({ size: 1024, bgColor: C.ink, onInk: true }),
      sizes: [1024, 512, 180],
    },
    {
      title: 'APPLE TOUCH · 180 PX', sub: 'iOS home screen',
      slug: 'padelmatch-appletouch',
      stage: 'light',
      build: () => PM.buildAppIconSVG({ size: 180, bgColor: C.cream }),
      sizes: [180],
    },
    {
      title: 'AVATAR · CREAM', sub: 'profile placeholder',
      slug: 'padelmatch-avatar-cream',
      stage: 'light',
      build: () => PM.buildAvatarSVG({ size: 512, bgColor: C.cream }),
      sizes: [512, 256, 128],
    },
    {
      title: 'AVATAR · INK', sub: 'profile placeholder · dark',
      slug: 'padelmatch-avatar-ink',
      stage: 'dark',
      build: () => PM.buildAvatarSVG({ size: 512, bgColor: C.ink, onInk: true }),
      sizes: [512, 256, 128],
    },
  ];
  ...
  ogCfgs.forEach(og => {
    ...
    // Scale down by CSS transform to fit preview
    tile.innerHTML = `
      <div class="stage">
  ...
  const TYPES = [
    {
      ...
      demo: `
        <div style="font-family:var(--ub);font-size:64px;font-weight:900;color:var(--lime);letter-spacing:-0.04em;line-height:0.95;">7.2</div>
  ...
  document.getElementById('anim-ink').innerHTML =
    PM.buildAnimatedSVG({ onInk: true, bgColor: null, width: 280 }) +
    PM.buildAnimatedSVG({ onInk: true, bgColor: null, width: 140 });
  document.getElementById('anim-cream').innerHTML =
    PM.buildAnimatedSVG({ onInk: false, bgColor: null, width: 280 }) +
    PM.buildAnimatedSVG({ onInk: false, bgColor: null, width: 140 });
  ...
  document.getElementById('dl-all-svg').addEventListener('click', async (e) => {
    const all = [
      ['padelmatch-mark-cream.svg', PM.buildLogoSVG({ cellFills: { TL: C.lime, BR: C.blue }, bgColor: C.cream, width: 512, includeXMLDecl: true })],
      ['padelmatch-mark-ink.svg',   PM.buildLogoSVG({ cellFills: { TL: C.lime, BR: C.blue }, onInk: true, bgColor: C.ink, width: 512, includeXMLDecl: true })],
  ...
  document.getElementById('dl-all-png').addEventListener('click', async (e) => {
    ...
    const variants = [
      { slug: 'padelmatch-mark-cream', opts: { cellFills: { TL: C.lime, BR: C.blue }, bgColor: C.cream } },
      { slug: 'padelmatch-mark-ink',   opts: { cellFills: { TL: C.lime, BR: C.blue }, onInk: true, bgColor: C.ink } },
  ...
  document.getElementById('dl-all-icons').addEventListener('click', async (e) => {
    ...
    // OG cards via foreignObject html
    for (const onInk of [false, true]) {
      const html = PM.buildOGCardHTML({ width: 1200, height: 630, onInk });
      const blob = await PM.htmlToPNGBlob(html, 1200, 630, onInk ? C.ink : C.cream);
  ...
  // ──────────────────────────────────────────────────────────────
  //  ASSET CARD BUILDER (for section 01)
  // ──────────────────────────────────────────────────────────────
  function makeAssetCard({ title, sub, slug, bg, opts, sizes = [256, 512, 1024] }) {
    const card = document.createElement('div');
    card.className = 'card';
    const previewSVG = PM.buildLogoSVG(opts).replace(/width="\d+" height="\d+"/, 'width="220"');
    card.innerHTML = `
      <div class="head">
  ...
  // ──────────────────────────────────────────────────────────────
  //  GLOBAL CLICK HANDLER (for wordmark/lockup/icon/og buttons)
  // ──────────────────────────────────────────────────────────────
  document.body.addEventListener('click', async (e) => {
    ...
    if (a === 'png' && btn.dataset.wm) {
      ...
      btn.textContent = '…';
      const html = PM.buildWordmarkHTML({ ...wm, size: Math.round(size * 0.18) });
      // pad
      const padX = Math.round(size * 0.05);
      const padY = Math.round(size * 0.20);
      const w = size; const h = Math.round(size * 0.33);
      const wrapped = `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;padding:${padY}px ${padX}px;background:#F5F2E8;box-sizing:border-box;">${html}</div>`;
      const blob = await PM.htmlToPNGBlob(wrapped, w, h, C.cream);
      PM.downloadBlob(blob, `${slug}@${size}.png`);
      btn.textContent = `↓ PNG @${size}`;
      PM.flashBtn(btn, 'DOWNLOADED');
      return;
    }
    ...
    if (a === 'lockup-png' && btn.dataset.cfg) {
      ...
      const size = +btn.dataset.size;
      btn.textContent = '…';
      // Scale markSize proportionally to output width; wordmarkSize stays null so it auto-fits to mark height
      const exportOpts = { ...cfg.lockupOpts };
      // Scale markSize from the cfg's reference (treating cfg.lockupOpts.markSize as the design size)
      const refMark = exportOpts.markSize || 96;
      exportOpts.markSize = Math.round(size * 0.20);
      // If the cfg had an explicit wordmarkSize (e.g. stacked variants), scale it too; if null, leave null to auto-compute
      if (exportOpts.wordmarkSize != null) {
        exportOpts.wordmarkSize = exportOpts.wordmarkSize * (exportOpts.markSize / refMark);
      }
      if (exportOpts.gap != null) {
        exportOpts.gap = exportOpts.gap * (exportOpts.markSize / refMark);
      }
      const lockupHtml = PM.buildLockupHTML(exportOpts);
      const padY = Math.round(size * 0.10);
      const padX = Math.round(size * 0.05);
      const w = size; const h = Math.round(size * (exportOpts.showTagline ? 0.50 : 0.42));
      const bg = cfg.bg === 'ink' ? C.ink : C.cream;
      const wrapped = `<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;padding:${padY}px ${padX}px;background:${bg};box-sizing:border-box;">${lockupHtml}</div>`;
      const blob = await PM.htmlToPNGBlob(wrapped, w, h, bg);
    ...
    if (a === 'og-png' && btn.dataset.slug) {
      ...
      btn.textContent = '…';
      const html = PM.buildOGCardHTML({ width: w, height: h, onInk });
      const blob = await PM.htmlToPNGBlob(html, w, h, onInk ? C.ink : C.cream);
      PM.downloadBlob(blob, `${btn.dataset.slug}-${w}x${h}.png`);
      btn.textContent = `↓ PNG · ${w}×${h}`;
      PM.flashBtn(btn, 'DOWNLOADED');
      return;
    }
  });
